...
export class CustomerAddress extends IObject {
...
const customerAddressApi = new ObjectApi<CustomerAddress>({
  url: '/api/v1/customer_address',
  objectConstructor: CustomerAddress,
  service: 'ums-t',
});

class CustomerAddressStore extends ObjectStore<CustomerAddress> {
  api: ObjectApi<CustomerAddress>;
  watchApi: WatchApi<CustomerAddress>;
  constructor(api: ObjectApi<CustomerAddress>, watchApi: WatchApi<CustomerAddress>) {
    super();
    this.api = api;
    this.watchApi = watchApi;
  }
...
export const customerAddressStore = new CustomerAddressStore(
  customerAddressApi,
  new DefaultWatchApi(),
);
